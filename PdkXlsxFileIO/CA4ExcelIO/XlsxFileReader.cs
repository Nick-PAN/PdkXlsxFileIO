...
namespace CA4ExcelIO
{
    class XlsxFileReader
    {
        ...
        private static void UnzipFile(string zipFileName, string targetDirectory)
        {
        ...
        private static int ProgressPercent(int currentRowS, int startRowSn, int finialRowSn)
        {
            return (currentRowS - startRowSn) * 100 / (finialRowSn - startRowSn);
        }
                        ...
                        else if (reader.NodeType == XmlNodeType.EndElement)
                        {
                            if (reader.Name == "row")
                            {
                                if (rowSN >= (isFirstRowColumnNames ? rowStartSN + 1 : rowStartSN))
                                {
                                    ...
                                    //发起进度变化事件
                                    newPercent=ProgressPercent(rowSN, rowStartSN, rowFinalSN);
        ...
        public DataSet Read(string fileName, bool isFirstRowColumnNames)
        {
            ...
            foreach (FileInfo sfi in sheetsFileInfoArr)
            {
                DataTable dt = ReadWorksheetFile(sfi.FullName, stringTable, isFirstRowColumnNames);
